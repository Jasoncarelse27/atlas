...
function checkFeatureAccess(feature, tier) {
  ...
  if (tier === 'free') {
    if (feature === 'camera') {
      return {
        allowed: false,
        message: 'Camera features require Atlas Studio (Claude Opus – $149.99/mo). Upgrade to unlock the full experience.',
  ...
  if (tier === 'core' && feature === 'camera') {
    return {
      allowed: false,
      message: 'Camera features are exclusive to Atlas Studio (Claude Opus – $149.99/mo). Upgrade to unlock.',
      suggestedTier: 'studio'
    };
  }
