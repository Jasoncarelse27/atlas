    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          // ✅ FIX: Use ONLY attachments array for all media (audio, image, etc.)
          attachments: message.audioUrl 
            ? [{type: 'audio', url: message.audioUrl}] 
            : message.imageUrl 
            ? [{type: 'image', url: message.imageUrl}]
