...
import { useMarkNotificationRead } from '../../hooks/useAgentsDashboard';
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Bell className="w-5 h-5 text-[#B2BDA3] dark:text-[#B2BDA3]" />
            <span className="font-semibold text-gray-900 dark:text-white">
              Notifications
            </span>
          </div>
          {unreadCount > 0 && (
            <span className="px-2 py-1 bg-[#B2BDA3] text-white text-xs font-semibold rounded-full">
              {unreadCount}
            </span>
          )}
        </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-2 py-0.5 text-xs font-medium rounded ${getTypeColor(notification.type)}`}>
                        {notification.type.replace('email_agent.', '')}
                      </span>
                      {!notification.is_read && (
                        <span className="w-2 h-2 bg-[#B2BDA3] rounded-full"></span>
                      )}
                    </div>
                  ...
                  {!notification.is_read && (
                    <button
                      ...
                      className="flex-shrink-0 p-2 text-gray-400 hover:text-[#B2BDA3] hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
